                                ...
                                if (valueIsUndefined) {
                                    if (messageDataJson.op == "makeEquipment") {
                                        ...
                                        conSQLConfig_1.conSQLConfig.selectEquipmentListByEquipmentHash(db, newEquipmentHash, srcDataAddres).then((newRes) => {
                                            ...
                                            if (retNewRes[4].length >= 10) {
                                                conSQLConfig_1.conSQLConfig.selectEquipmentListByEquipmentHash(db, retNewRes[4], srcDataAddres).then((pastRes) => {
                                                    lcInitConfig_1.lcInitConfig.updateARbyAddresToEquipmentPosition(db, addres, newEquipmentHash, retNewRes[1], retNewRes[2], retNewRes[3], "+", pastRes.equipmentValueType, pastRes.equipmentValue, "-").then((result) => {
                                                    }).catch((error) => { console.error(error); });
                                                });
                                            }
                                            else {
                                                lcInitConfig_1.lcInitConfig.updateARbyAddresToEquipmentPosition(db, addres, newEquipmentHash, retNewRes[1], retNewRes[2], retNewRes[3], "+", "", "", "").then((result) => {
                                                }).catch((error) => { console.error(error); });
                                            }
                                            if (retNewRes[5] == addres) {
                                            }
                                        });
                                    }
                                }
